...
def parse_args():
    parser = argparse.ArgumentParser(description="Train U-Net on a VOC-style segmentation dataset.")
    parser.add_argument("--vocdevkit-path", default=None)
    parser.add_argument("--num-classes", type=int, default=None)
    parser.add_argument("--backbone", choices=["vgg", "resnet50", "mobilenetv2"], default=None)
    parser.add_argument("--attention-type", default=None)
    ...
    parser.add_argument("--init-epoch", type=int, default=None)
    parser.add_argument("--freeze-epoch", type=int, default=None)
    parser.add_argument("--unfreeze-epoch", type=int, default=None)
    parser.add_argument("--freeze-batch-size", type=int, default=None)
    parser.add_argument("--unfreeze-batch-size", type=int, default=None)
    parser.add_argument("--freeze-train", type=str2bool, default=None)
    parser.add_argument("--init-lr", type=float, default=None)
    parser.add_argument("--optimizer-type", choices=["adam", "sgd"], default=None)
    parser.add_argument("--dice-loss", type=str2bool, default=None)
    parser.add_argument("--focal-loss", type=str2bool, default=None)
...
if __name__ == "__main__":
    ...
    log_dir_root = save_dir
    if args.vocdevkit_path is not None:
    ...
    if args.freeze_batch_size is not None:
        Freeze_batch_size = args.freeze_batch_size
    if args.unfreeze_batch_size is not None:
    ...
    model = Unet(num_classes=num_classes, pretrained=pretrained, backbone=backbone, attention_type=attention_type).train()
    if not pretrained:
        weights_init(model)
    if model_path != '':
        #------------------------------------------------------#
    ...
    if local_rank == 0:
        ...
        log_dir         = os.path.join(log_dir_root, "loss_" + str(time_str))
    ...
    if local_rank == 0:
        show_config(
            num_classes = num_classes, backbone = backbone, attention_type = attention_type, model_path = model_path, input_shape = input_shape, \
