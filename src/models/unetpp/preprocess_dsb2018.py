...
def main():
    ...
    paths = glob('inputs/data-science-bowl-2018/stage1_train/*')  # glob 被用来查找指定目录下的所有文件和文件夹，使用通配符 * 表示匹配任何文件或文件夹名称
    ...
    for i in tqdm(range(len(paths))):
        path = paths[i]
        img = cv2.imread(os.path.join(path, 'images',  # basename(path) 用于获取 path 中的最后一部分，通常是文件夹的名称
