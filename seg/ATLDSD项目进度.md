...
# 2026-06-08 论文创新指南 2026 反馈后的计划升级
...
```text
当前所有“继续加模块”的尝试都没有超过 Boundary1，甚至下降。
因此 CFR 不再作为唯一优先结构创新。
CFR 的 component feedback 思想保留，但优先作为 CHFR / CMF 的 gate 使用。
```

从 PDF 借鉴的具体点：

```text
1. xx-Former = Token Mixer + FFN:
   不推翻 DeepLabV3+，只在 ASPP 后或 decoder feature 上改 token mixer / refinement。

2. A+B / A+B+C 双分支:
   CNN 负责局部纹理和边界，Mamba 负责低分辨率全局上下文，FFT/高频负责小病斑纹理。

3. 低分辨率全局建模:
   Mamba 只允许在 1/16 ASPP feature 上做，禁止完整 CNN+Mamba 双主干。

4. 高频/频域小目标增强:
   小病斑更像高频边界问题，不能只靠 LGLC 这种上下文模块。

5. 自适应融合:
   用 lesion / boundary / center 辅助头生成 gate，而不是简单 concat 或相加。
```
...
参考低分辨率 query / local + pooled query:
...
```text
1. Boundary1-384
   不加新模块，只验证高分辨率是否保住小病斑。

2. Boundary1-repeat
   跑 seed 42 / 2026，确认 Boundary1 是否稳定。

3. CHFR-256
   Component-guided High-Frequency Refinement。
   LBSB 后、cls_conv 前，引入高频/频域细化，并用 component cues 门控。
   这是 PDF 反馈后的第一结构优先级。

4. CHFR-384
   只有 Boundary1-384 或 CHFR-256 接近成功才跑。

5. CMF-256
   Component-guided CNN-Mamba-Frequency Fusion。
   ASPP 后、decoder concat 前，低分辨率 CNN + Mamba + frequency 三分支融合。
   这是高风险候选，不抢 CHFR 优先级。

6. CMF-384 / seed repeat
...
```text
目标:
把训练集和验证集切小，用 1 分钟级快筛判断 PlugNPlay 模块是否有涨点信号。
...
train = 64, val = 32
train disease presence: rust 12, alternaria 13, gray 11, brown 10
val disease presence: rust 7, alternaria 6, gray 7, brown 5

seed23:
D:\Code\ATLDSD\outputs\fast_voc\VOCdevkit_fast_64_32_seed23
train = 64, val = 32
train disease presence: rust 12, alternaria 13, gray 12, brown 8
val disease presence: rust 7, alternaria 6, gray 6, brown 7
```
...
```text
LSK-ASPP:
  seed11 mIoU = 37.52, FG = 26.71
  seed23 mIoU = 38.13, FG = 26.92
  两 seed 均值: mIoU = 37.83, FG = 26.82
  相对 baseline 均值: +1.33 mIoU, +1.30 FG
  判定: 优先升级到中程快筛。

CPCA-ASPP:
  seed11 mIoU = 37.76, FG = 26.79
  seed23 mIoU = 36.18, FG = 24.78
  两 seed 均值: mIoU = 36.97, FG = 25.78
  相对 baseline 均值: +0.47 mIoU, +0.26 FG
  判定: 弱候选，暂不进入正式长训。

EMA-ASPP:
  seed11 mIoU = 36.05, FG = 24.70
  判定: 淘汰。

SCSA-ASPP:
  seed11 mIoU = 35.36, FG = 23.96
  判定: 淘汰。
```
...
```text
先跑 LSK-ASPP 的 24 或 32 epoch 中程快筛，仍用 seed11/seed23 同 seed baseline 对照。
如果 LSK 中程仍稳定，再扩大到 128/64 或 192/64 快筛。
只有扩大快筛仍稳定，才启动正式 150 epoch。
```

## 2026-06-09 LSK-ASPP 中程复验结果

24 epoch 中程快筛已经推翻 12 epoch 的 LSK 正信号。
...
seed = 11 / 23
epoch = 24
anchor = Boundary1 + LBSB
变量 = ASPP attention none vs LSK
```

两 seed 均值：

| 组 | avg mIoU | avg FG mIoU | avg Severity MAE | avg Grade Acc | avg rust IoU | avg alternaria IoU | avg brown IoU | 判断 |
|---|---:|---:|---:|---:|---:|---:|---:|---|
| baseline | 47.26 | 37.85 | 0.03210 | 79.68 | 62.20 | 28.61 | 12.28 | 对照 |
| LSK-ASPP | 45.70 | 36.06 | 0.02757 | 79.69 | 60.62 | 16.40 | 17.40 | mIoU/FG 失败 |
| LSK - baseline | -1.56 | -1.79 | -0.00453 | +0.01 | -1.58 | -12.21 | +5.12 | 停止升级 |

判定：

```text
LSK-ASPP 不跑 128/64，不跑 150 epoch。
它可以保留为负消融: 大核选择性上下文可能改善 brown/severity，但在中程训练中损害主分割，尤其 alternaria。
```

新的当前下一步：

```text
继续 64/32、12ep、seed11/23 快筛下一批轻量 PlugNPlay 候选:
1. SK-ASPP
2. Triplet-ASPP
3. SP-ASPP / strip_pooling

晋级门槛:
两 seed 平均 mIoU 和 FG mIoU 同时高于 baseline，才进入 24ep 中程复验。
```

## 2026-06-09 SP-ASPP 中程复验与 ASPP 插件路线调整

LSK-ASPP 失败后，继续筛 `SK-ASPP`、`Triplet-ASPP`、`SP-ASPP`。12ep 双 seed 中只有 `SP-ASPP` 达到晋级门槛：

```text
12ep SP-ASPP:
avg mIoU = 38.20, baseline = 36.50, delta = +1.70
avg FG mIoU = 27.42, baseline = 25.52, delta = +1.90
```
...
|---|---:|---:|---:|---:|---:|---:|---:|---|
| baseline | 47.26 | 37.85 | 0.03210 | 79.68 | 62.20 | 28.61 | 12.28 | 对照 |
| SP-ASPP | 46.70 | 37.25 | 0.03146 | 82.81 | 59.62 | 23.80 | 17.74 | mIoU/FG 失败 |
| SP - baseline | -0.56 | -0.60 | -0.00064 | +3.13 | -2.58 | -4.81 | +5.46 | 停止升级 |

结论：

```text
SP-ASPP 不跑 128/64，不跑 150 epoch。
ASPP attention 路线连续出现 12ep 假阳性:
  LSK-ASPP: 12ep 正，24ep 失败。
  SP-ASPP: 12ep 正，24ep 失败。

下一轮改为 decoder/refinement 插入点:
1. SP-Decoder
2. SCSE-Decoder 或 SimAM-Decoder

如果 decoder 插件仍不能通过 24ep 双 seed，停止普通 PlugNPlay attention 拼装，回到 CHFR / CMF 机制线。
```
